...
class Emojifier {
    ...
    // Creating Threshold constants for eye being openned and smiling
    private static final double SMILING_PROBABILITY_THRESHOLD = .10;
    private static final double EYE_OPEN_PROBABILITY_THRESHOLD = .5;
    ...
    private static void whichEmoji(Face face){
        ...
        // (AT TOP)COMPLETED (3): Create threshold constants for a person smiling, and and eye being open by taking pictures of yourself and your friends and noting the logs.
        // COMPLETED (4): Create 3 boolean variables to track the state of the facial expression based on the thresholds you set in the previous step: smiling, left eye Opened, right eye Opened.
        boolean smilingFace = face.getIsSmilingProbability() > SMILING_PROBABILITY_THRESHOLD;
        boolean leftEyeClosed = face.getIsLeftEyeOpenProbability() < EYE_OPEN_PROBABILITY_THRESHOLD;
        boolean rightEyeClosed = face.getIsRightEyeOpenProbability() < EYE_OPEN_PROBABILITY_THRESHOLD;
        // COMPLETED (5): Create an if/else system that selects the appropriate emoji based on the above booleans and log the result.
        // Determining and loging the appropriate emoji
        Emoji emoji;
        if (smilingFace){
